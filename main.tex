...
\begin{algorithm}
    ...
    \For{n=1,2,..,N}{
        Sample \(\theta^{(n)} \sim \gamma_P^{\theta}(x_{1:P}^{(n-1)}(2),x_P^{(n-1)}(1))\). If directly sampling is not possible, \(\theta^{(n)}\) can be obtained by e.g. applying Metropolis-Hastings kernel\;
